...
module.exports = function (options) {
    ...
    return function (req, res, next) {
        ...
        var roots = [].concat(req.app.get('views')).concat(options.viewsDirectory);
        ...
        try {
            PARTIALS.forEach(function (relativeTemplatePath) {
                ...
                var viewExtension = '.' + options.viewEngine;
                var engines = {};
        ...
        var getTemplate = function getTemplate(child) {
            ...
            if (match) {
                return fs.readFileSync(res.locals.partials['partials-' + match[1]] + '.' + options.viewEngine).toString();
            } else if (child === 'html' || res.locals[child]) {
                var panelPath = path.join(options.viewsDirectory, PANELMIXIN + '.' + options.viewEngine);
                var localsPanelPath = path.join(res.locals.partials['partials-mixins-panel'] + '.' + options.viewEngine);
            ...
            return function () {
                if (this.child) {
                    ...
                    return template.render(Object.assign({
                        ...
                        return Hogan.compile(fs.readFileSync(path + '.' + options.viewEngine).toString());
                    }));
                }
            };
        }
                ...
                handler: function () {
                    ...
                    */
                    return function (key) {
                        ...
                        yearPart = compiled['partials/forms/input-text-group'].render(inputText.call(this, key + '-year', { pattern: '[0-9]*', maxlength: 4, hintId: key + '-hint', date: true, formGroupClassName: 'form-group-year', autocomplete: autocomplete.year }));
